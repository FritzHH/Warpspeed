// Cadence Extension — popup logic (cross-browser).
//
// Two views:
...
// renders status.
//
// Cross-browser: imports webextension-polyfill so `browser.*` works
// identically in Chrome, Firefox, and Safari. The polyfill is a no-op where
// `browser` already exists natively (FF, Safari) and bridges `chrome.*` in
// Chrome.
...
async function render() {
  clearErr();
  const authResp = await browser.runtime.sendMessage({ type: "getAuth" });
...
$("saveSettingsBtn").addEventListener("click", async () => {
  ...
  await browser.storage.local.set({ settings: { tenantID, storeID } });
