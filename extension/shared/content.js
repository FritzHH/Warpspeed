// Cadence Extension — content script (cross-browser).
//
...
//
// Cross-browser: webextension-polyfill normalizes `browser.*` across Chrome,
// Firefox, and Safari. Built with esbuild as an IIFE so the polyfill +
// config imports inline cleanly into a single content-script bundle.

import browser from "webextension-polyfill";

const INJECTED_ATTR = "data-cadence-injected";
const PANEL_HOST_ID = "cadence-extension-panel-host";
// Persisted boolean in browser.storage.local. When true, every fresh page load
...
// Vendor filter for the order list. Values: "all" | "jbi" | "qbp". Reset to
// the current site's catalog on every page load (see openSidePanel) — picking
// a different filter only lasts for the current page. Initialized to "all" as
// a safe pre-mount placeholder.
let panelVendorFilter = "all";

// Extension-relative URL for the trash icon (extension/shared/assets/trash.webp).
// Resolved here so the row templates can interpolate it as a string. Declared
// via `web_accessible_resources` in each platform's manifest so vendor pages
// can load it from chrome-extension://… URLs.
const TRASH_ICON_URL = browser.runtime.getURL("assets/trash.webp");
...
const VENDOR_ADAPTERS = {
  "www.jbi.bike": {
    ...
    findAddButtons(root) {
      // search_usd.php rows ship a canonical <button class="btn-add-to-order"
      // id="<partNumber>">. product_details.php may render the same control
      // as an input[type=submit] (or a button without the .btn-add-to-order
      // class) — cast a wider net on the detail page so missing/renamed
      // classes don't silently leave the button uninjected.
      const primary = Array.from(
        root.querySelectorAll("button.btn-add-to-order")
      );
      if (!location.pathname.includes("/product_details.php")) return primary;
      const seen = new Set(primary);
      const candidates = root.querySelectorAll(
        'button[id], input[type="submit"][id], input[type="button"][id]'
      );
      candidates.forEach((el) => {
        if (seen.has(el)) return;
        const text = (el.textContent || el.value || "").trim().toLowerCase();
        if (text.includes("add to order")) {
          primary.push(el);
          seen.add(el);
        }
      });
    ...
    getPartNumber(btn) {
      // JBI stamps the bare part number on the button's id attribute (works
      // for both <button> and <input> elements found by findAddButtons).
      return btn.id || "";
    },
    getRow(btn) {
      // Detail page lacks the .row containers used on search; fall back to
      // a form or the main product container so extractCost still has a
      // meaningful subtree to scan.
      return (
        btn.closest(".row") ||
        btn.closest("form") ||
    ...
    getProductURL(row) {
      // Detail page: the page itself IS the product page.
      if (location.pathname.includes("/product_details.php")) return location.href;
      if (!row) return "";
      const a = row.querySelector('a[href*="product_details.php"]');
      if (!a) return "";
      try {
    ...
    findPriceElement(row) {
      // Smallest element in the row whose textContent matches "$X.XX ea" —
      // walking children narrows from the row itself (which trivially
      // matches) to the actual price label (.price span or sibling text).
      if (!row) return null;
      const re = /\$\d+\.\d{2}\s*ea/;
      let best = null;
    ...
    getQtyEl(pn) {
      // search_usd.php: <input id="qty_<part>"> is the canonical control.
      let el = document.getElementById(`qty_${pn}`);
      if (el) return el;
      // Detail-page / markup-variant fallback: same name shape but the
      // element may not carry the id.
      el = document.querySelector(`input[name="qty_${pn}"]`);
      if (el) return el;
      // Last-ditch: scope to the part's add button and pick the nearest qty
      // input within the same form/row/container.
      const addBtn = document.getElementById(pn);
      if (addBtn) {
        const scope =
          addBtn.closest("form") ||
          addBtn.closest(".row") ||
          addBtn.closest(".product-details") ||
          addBtn.closest("div");
        if (scope) {
          el =
            scope.querySelector('input[id^="qty"]') ||
            scope.querySelector('input[name^="qty"]') ||
            scope.querySelector('input[name="quantity"]') ||
            scope.querySelector("input#quantity");
          if (el) return el;
        }
      }
      return null;
    ...
    injectWrapper(addBtn, wrapper) {
      // product_details.php: single-product layout — wrapper goes INLINE to
      // the immediate right of the Add to Order button, not on a new line.
      // search_usd.php: rows with a btn-group — wrapper goes BELOW via
      // flex-wrap so it doesn't squeeze in beside the vendor's qty/ship
      // controls.
      if (location.pathname.includes("/product_details.php")) {
        ...
        if (addBtn.parentElement) {
          addBtn.parentElement.insertBefore(wrapper, addBtn.nextSibling);
        }
        return;
      }
  ...
  "www.qbp.com": {
    ...
    getProductURL(row) {
      // Detail page detection: #productPricing only exists on the detail page.
      if (document.getElementById("productPricing")) return location.href;
      if (!row) return "";
    ...
    getProductName(row) {
      // QBP catalog isn't imported to RTDB yet — without this scrape the
      // sidebar would show the raw part number (e.g. "LU0199") and $0.00.
      // Detail page: row is #productPricing (cart section), so look up to a
      // page-level heading. Listing page: the product link's text is the name.
      if (document.getElementById("productPricing")) {
        const heading =
...
function ensureSidePanel() {
  ...
  shadow.innerHTML = `
    <style>
      ...
      .vendor-filter.visible { display: flex; }
      ...
      .vendor-filter button:hover { background: #f3f4f6; }
      ...
      .btn-trash img { display: block; width: 18px; height: 18px; }
      .panel-footer {
      ...
      /* Per-line qty adjuster buttons — single horizontal row sitting
         immediately right of the cost field. Up on left, down on right. */
      .qty-adjusters {
        display: inline-flex;
        flex-direction: row;
        align-items: center;
    ...
    <div class="panel" id="panel">
      ...
      <div class="vendor-filter" id="vendorFilter" role="tablist" aria-label="Filter by vendor">
        <button type="button" data-vendor-filter="all" role="tab">All</button>
        <button type="button" data-vendor-filter="jbi" role="tab">JBI</button>
        <button type="button" data-vendor-filter="qbp" role="tab">QBP</button>
      </div>
  ...
  shadow.getElementById("vendorFilter").addEventListener("click", (ev) => {
    const btn = ev.target.closest("button[data-vendor-filter]");
    if (!btn) return;
    const next = btn.getAttribute("data-vendor-filter");
    if (!next || next === panelVendorFilter) return;
    panelVendorFilter = next;
    // Not persisted — the filter resets to the current site's vendor on every
    // page load (see openSidePanel). Picking "All" or the other vendor is a
    // per-page choice.
    refreshSidePanel();
  });
  ...
  // content script. browser.runtime is undefined in that state.
  shadow.getElementById("btnLogout").addEventListener("click", async () => {
    try {
      if (!browser.runtime || !browser.runtime.sendMessage) return;
...
async function openSidePanel() {
  ...
  // First mount: suppress both the panel's slide-in AND the body's
  // padding-right transition. The vendor page paints, then in the very next
  // frame the panel + body shift snap into place together — no 300ms
  // "sidebar pushes your page sideways while you're trying to read it"
  // animation. The page-and-sidebar pair lands as a single visual state.
  // Subsequent min/max toggles or close gestures still animate (transition
  ...
  // Vendor filter ALWAYS defaults to the current site's catalog on page load,
  // regardless of what the user last picked. Switching between jbi.bike and
  // qbp.com should auto-focus the matching tab — that's the common case. The
  // user can still pick "All" or the other vendor, but that choice only lasts
  // for the current page (not persisted across navigation).
  panelVendorFilter = VENDOR ? VENDOR.vendorCatalogID : "all";
  panel.classList.add("open");
  applyPanelPush(true, /* instant */ true);
  refreshSidePanel();
  // Restore the slide animation in the next frame so any subsequent close /
  // open / min-max toggle within this page lifetime still animates smoothly.
...
// browser.storage.local so every subsequent page load opens at the same size
...
function setSidePanelMinimized(minimized) {
  ...
  try {
    browser.storage.local.set({ [PANEL_MINIMIZED_KEY]: !!minimized });
  } catch (_err) {
...
function findPanelRowByVendorItemID(vendorItemID) {
  const shadow = getPanelShadow();
  if (!shadow) return null;
  return shadow.querySelector(
    `.item[data-vendor-item-id="${cssEscape(vendorItemID)}"]`
  );
}

// Optimistic placeholder for a brand-new line. Server-resolved fields (real id,
// display name, MSRP, cost-diff badge) get filled in by the refreshSidePanel
// that fires after the server confirms the add. Returns the temp id used as
// data-item-id so the caller can roll back by removing the same row.
function insertOptimisticPanelRow({ vendorItemID, vendorItemName, qty, costStr, vendorURL }) {
  const shadow = getPanelShadow();
  if (!shadow) return null;
  const orderList = shadow.getElementById("orderList");
  if (!orderList) return null;
  const empty = orderList.querySelector(".empty");
  if (empty) empty.remove();
  const tempID =
    "__cadence_optimistic_" + Math.random().toString(36).slice(2, 10);
  const costDollars = Number(costStr) || 0;
  const costCents = Math.round(costDollars * 100);
  const line = qty * costDollars;
  const displayName = (vendorItemName && String(vendorItemName).trim()) || vendorItemID;
  const safeName = String(displayName).replace(/</g, "&lt;");
  const vendorItemIDAttr = String(vendorItemID).replace(/"/g, "&quot;");
  const nameMarkup = vendorURL
    ? `<a class="item-name-link" href="${String(vendorURL).replace(/"/g, "&quot;")}" target="_blank" rel="noopener noreferrer">${safeName}</a>`
    : safeName;
  const row = document.createElement("div");
  row.className = "item";
  row.setAttribute("data-item-id", tempID);
  row.setAttribute("data-vendor-item-id", vendorItemIDAttr);
...
function setPanelRowQty(itemID, newQty) {
  ...
  if (meta) {
    const qtySpan = meta.querySelector(".qty-badge");
...
function applyPanelPush(open, instant) {
  ...
  // First-mount path: snap the body padding instantly so the panel and the
  // body shift land in the same paint frame. The transition is restored after
  // layout commits so subsequent min/max/close gestures still animate.
  if (instant) {
    document.body.style.transition = "none";
  } else if (!document.body.style.transition.includes("padding-right")) {
...
function syncVendorFilterUI(shadow, visible) {
  const bar = shadow.getElementById("vendorFilter");
  if (!bar) return;
  bar.classList.toggle("visible", !!visible);
  bar.querySelectorAll("button[data-vendor-filter]").forEach((b) => {
    b.classList.toggle(
      "active",
      b.getAttribute("data-vendor-filter") === panelVendorFilter
    );
  });
}
...
async function refreshSidePanel() {
  ...
  const authResp = await browser.runtime.sendMessage({ type: "getAuth" }).catch(() => null);
  ...
  // resets every badge back to blue. Always uses the unfiltered list — badges
  // reflect actual order content, not the panel's current view.
  setOrderQtyCache(items);

  if (!items.length) {
    meta.textContent = `0 items • updated ${addedDate}`;
  ...
  syncVendorFilterUI(shadow, true);

  const visibleItems =
    panelVendorFilter === "all"
      ? items
      : items.filter(
          (it) => String(it.vendorCatalogID || "").toLowerCase() === panelVendorFilter
        );

  if (panelVendorFilter === "all") {
    meta.textContent = `${items.length} item${items.length === 1 ? "" : "s"} • updated ${addedDate}`;
  } else {
    const label = panelVendorFilter.toUpperCase();
    meta.textContent = `${visibleItems.length} of ${items.length} ${label} • updated ${addedDate}`;
  }

  if (!visibleItems.length) {
    const label = panelVendorFilter.toUpperCase();
    orderList.innerHTML = `<div class="empty"><strong>No ${label} items in this order.</strong><br/>Switch to <em>All</em> to see everything.</div>`;
    total.textContent = "$0.00";
    return;
  }
  ...
  const rows = visibleItems.map((it) => {
    ...
    const vendorURL = String(display.vendorURL || "").trim();
    const nameMarkup = vendorURL
      ? `<a class="item-name-link" href="${vendorURL.replace(/"/g, "&quot;")}" target="_blank" rel="noopener noreferrer">${name}</a>`
      : name;
    ...
    return `
      <div class="item" data-item-id="${itemIDAttr}" data-vendor-item-id="${vendorItemIDAttr}" data-qty="${qty}" data-cost-cents="${costCentsAttr}">
        <div class="item-name">${nameMarkup}</div>
        <div class="item-meta">
          <button type="button" class="btn-trash" data-action="delete"
                  data-item-id="${itemIDAttr}" aria-label="Remove item">
            <img src="${TRASH_ICON_URL}" alt="" aria-hidden="true" />
          </button>
          <span class="qty-badge">${qty}</span>
          <span>@ $${costDollars.toFixed(2)}</span>
...
// qty adjuster / delete. SVG icons inside buttons swallow the click target, so
// we walk up to the nearest [data-action] element instead of reading the
// target directly.
function handlePanelBodyClick(ev) {
  ...
  if (!action) return;
  if (action === "apply") {
    handleApplyCostClick(actionEl);
  } else if (action === "info") {
    handleInfoClick(actionEl);
  } else if (action === "qty-up" || action === "qty-down") {
    handleQtyAdjustClick(actionEl, action === "qty-up" ? 1 : -1);
  } else if (action === "delete") {
    handleDeleteClick(actionEl);
  }
}
...
// own deletion — user can remove items in the app). Patches the row + footer
// total + on-page badge IMMEDIATELY (optimistic), then fires the server
// callable in the background. Buttons stay enabled so rapid clicks compound;
// each subsequent click reads the freshly-updated data-qty and fires an
// absolute qty. On server error we fully refresh from source of truth — this
// also correctly handles out-of-order responses from spam-clicks.
async function handleQtyAdjustClick(btn, delta) {
  ...
  // Optimistic DOM patch first — no awaits between click and visual change.
...
// Delete a single order line. Optimistic: removes the row, updates the on-page
// "Added [N]" badge, recomputes panel total — all instantly. Server call fires
// in the background; on failure we refresh from source of truth to reconcile.
async function handleDeleteClick(btn) {
  ...
  const pn = row.dataset.vendorItemId || "";
  const removedQty = Number(row.dataset.qty || 0);

  row.remove();
  if (pn) {
    const prior = orderQtyByPn[pn] || 0;
    const next = Math.max(0, prior - removedQty);
    if (next > 0) orderQtyByPn[pn] = next;
...
function injectButtonsInTree(root) {
  ...
  adds.forEach((addBtn) => {
    ...
    injectQtyChevrons(addBtn, partNumber);
  });
}

// Injects ▲/▼ chevron buttons immediately right of the vendor's price label
// (currently JBI only). Each click mutates the vendor's qty input + fires
// input/change events so any vendor-side listener reacts as if typed. The
// ▼ button auto-disables at qty=1. Marked idempotent via dataset flag so
// MutationObserver replays don't dupe the buttons.
function injectQtyChevrons(addBtn, partNumber) {
  if (!VENDOR || typeof VENDOR.findPriceElement !== "function") return;
  const row = VENDOR.getRow(addBtn);
  const priceEl = VENDOR.findPriceElement(row);
  if (!priceEl) return;
  if (priceEl.dataset.cadenceChevrons === "1") return;
  const qtyEl = VENDOR.getQtyEl(partNumber);
  if (!qtyEl) return;
  priceEl.dataset.cadenceChevrons = "1";

  const adj = document.createElement("span");
  adj.className = "cadence-qty-chevrons";
  adj.style.cssText = `
    display: inline-flex;
    flex-direction: column;
    margin-left: 6px;
  ...
  const refreshDisabled = () => {
    ...
    const current = Number.isFinite(v) && v >= 1 ? v : 1;
    const atFloor = current <= 1;
    down.disabled = atFloor;
    down.style.opacity = atFloor ? "0.4" : "1";
    down.style.cursor = atFloor ? "default" : "pointer";
  };

  const bump = (delta) => {
    const v = Number(qtyEl.value);
    const base = Number.isFinite(v) && v >= 1 ? v : 1;
  ...
  // Keep ▼ disabled-state in sync with manual edits to the qty input.
  qtyEl.addEventListener("input", refreshDisabled);
  qtyEl.addEventListener("change", refreshDisabled);

  adj.appendChild(up);
  adj.appendChild(down);
  if (priceEl.parentElement) {
    priceEl.parentElement.insertBefore(adj, priceEl.nextSibling);
  } else {
    priceEl.appendChild(adj);
  }
  refreshDisabled();
}
...
async function handleAddClick(vendorAddBtn, ourBtn, partNumber) {
  ...
  // Honor whatever the vendor's qty input says; blank / non-numeric / ≤0
  // all silently default to 1 so a one-click add never fails on "no qty".
  const rawQty = qtyEl ? Number(qtyEl.value) : NaN;
  ...
  const vendorURL =
    typeof VENDOR.getProductURL === "function" ? VENDOR.getProductURL(row) : "";
  const vendorItemName =
    typeof VENDOR.getProductName === "function" ? VENDOR.getProductName(row) : "";
  ...
  // Optimistic UI: paint the click as immediately successful. The button flips
  // green with the post-add qty badge; the side panel bumps a matching row's
  // qty (when one exists for this part number) or inserts a placeholder row.
  // The server request fires in the background; on failure, all three pieces
  // of state roll back to their pre-click values.
  const priorQty = orderQtyByPn[partNumber] || 0;
  const newCachedQty = priorQty + qty;
  orderQtyByPn[partNumber] = newCachedQty;
  updateInjectedButtonState(ourBtn, newCachedQty);

  let bumpedRowItemID = null;
  let bumpedRowPriorQty = 0;
  let optimisticRowID = null;
  const filterAllows =
    panelVendorFilter === "all" ||
    panelVendorFilter === VENDOR.vendorCatalogID;
  if (panelIsOpen() && filterAllows) {
    const existing = findPanelRowByVendorItemID(partNumber);
    if (existing) {
      bumpedRowItemID = existing.getAttribute("data-item-id") || null;
      bumpedRowPriorQty = Number(existing.getAttribute("data-qty") || 0);
      if (bumpedRowItemID) {
        setPanelRowQty(bumpedRowItemID, bumpedRowPriorQty + qty);
      }
    } else {
      optimisticRowID = insertOptimisticPanelRow({
  ...
  const rollback = (msg, kind = "err") => {
    if (priorQty > 0) orderQtyByPn[partNumber] = priorQty;
    else delete orderQtyByPn[partNumber];
    updateInjectedButtonState(ourBtn, priorQty);
    if (bumpedRowItemID) setPanelRowQty(bumpedRowItemID, bumpedRowPriorQty);
    if (optimisticRowID) removeOptimisticPanelRow(optimisticRowID);
    flashFeedback(ourBtn, msg, kind);
  };
  ...
  try {
    ...
    rollback("Error: " + (err && err.message));
    return;
  }

  if (!resp || !resp.ok) {
    rollback("Error: " + (resp && resp.error));
    return;
  }
  ...
  if (!result.success) {
    ...
    if (reason === "not_signed_in") {
      rollback("Sign in via the side panel", "warn");
      openSidePanel();
    } else if (reason === "no_active_order") {
      rollback("Create an order in Cadence first", "warn");
      openSidePanel();
    } else if (reason === "active_order_missing") {
      rollback("Active order missing — pick another in Cadence", "warn");
      openSidePanel();
    } else {
      rollback("Failed: " + (result.message || reason));
    }
    return;
  }
