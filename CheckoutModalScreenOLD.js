...
export function CheckoutModalScreen({}) {
  // store getters /////////////////////////////////////////////////////////
  const zReceiptScanMessage = useCheckoutStore((state) => state.message);
  ...
  useEffect(() => {
    ...
    // testing /////////////////////////
    useCheckoutStore.getState().setStringOnly(refundScan);
  ...
  function handlePaymentCapture(payment = PAYMENT_OBJECT_PROTO) {
    ...
    sCombinedWorkorders.forEach((wo) => {
      ...
      useOpenWorkordersStore.getState().setWorkorder(wo); // db
    });
  ...
  function handleRefundScan(refundScan) {
    useCheckoutStore.getState().setReceiptScan(refundScan, (workorders, sale) => {
          ...
          <View style={{ width: "39%" }}>
            ...
            {sSearchString.length > 1 ? (
              ...
              <WorkorderListComponent
                ...
                zCustomer={zCustomer}
                zSettings={zSettings}
                _zGetInventoryItem={useInventoryStore.getState().getInventoryItem}
